"""
TrackFood_AutoRecover.py — SAM2 自动追踪恢复版

新功能：
1. 小批量处理（CHUNK_SIZE=12，约2秒延迟）
2. Mask质量监控 + 自动恢复
3. RGB+IR联合检测新食材入锅
4. 支持无标注模式（完全自动化）

流程：
1. 读取 food_labels.json（可选，无标注时自动检测）
2. 小批量追踪（每批12帧）
3. 批间检查mask质量，丢失时自动恢复
4. 检测新食材入锅，自动追加追踪目标
5. 输出温度数据 + 可视化视频
...
# 导入自动追踪工具
from auto_tracking_utils import (
    TrackingState,
    check_mask_quality,
    auto_recover_tracking,
    detect_new_food_combined
)
...
OUTPUT_VIDEO    = os.path.join(_HERE, "..", "output", "track_result_auto.mp4")
OUTPUT_CSV      = os.path.join(_HERE, "..", "output", "food_temp_log_auto.csv")
...
# 分批处理参数（核心：避免 OOM）
CHUNK_SIZE      = 12      # 每批处理帧数，12帧约2秒延迟（25 FPS）
...
# ── 自动化参数 ────────────────────────────────────────────────────────────────
AUTO_RECOVER_ENABLED = False     # 启用自动恢复（手持拍摄阶段建议关闭，固定到锅上后再启用）
AUTO_DETECT_ENABLED  = True      # 启用新食材检测

# Mask质量阈值
MASK_LOST_THRESHOLD  = 1.0       # mask_ratio < 1% 视为丢失
MASK_WARN_THRESHOLD  = 0.5       # mask_ratio < 0.5% 警告
CONSECUTIVE_LOST     = 2         # 连续2批低于阈值触发恢复

# RGB检测阈值
RGB_DIFF_THRESHOLD   = 30        # 灰度差 > 30 视为运动
RGB_MIN_AREA         = 500       # 最小区域面积（像素）
RGB_ASPECT_RATIO_MAX = 3.0       # 长宽比 < 3 视为团状

# IR温度验证阈值（根据实际测试调整，可能需要提高到5-10°C）
IR_TEMP_DIFF_MIN     = 3.0       # 新区域比周围冷N°C以上（建议测试后调整）
IR_MIN_PIXELS        = 50        # 映射到IR后最小像素数

# 前景点生成
FG_POINTS_MIN        = 10        # 最少前景点
FG_POINTS_MAX        = 50        # 最多前景点
...
def main():
    ...
    # ── 初始化追踪状态管理器 ──────────────────────────────────────────────────
    tracking_state = TrackingState(window_size=3)
    print(f"\n[自动化] 自动恢复: {'启用' if AUTO_RECOVER_ENABLED else '禁用'}")
    print(f"[自动化] 新食材检测: {'启用' if AUTO_DETECT_ENABLED else '禁用'}")
    print(f"[自动化] Mask丢失阈值: {MASK_LOST_THRESHOLD}%  连续{CONSECUTIVE_LOST}批")
    print(f"[自动化] IR温差阈值: {IR_TEMP_DIFF_MIN}°C")
    ...
    for chunk_i in range(n_chunks):
        ...
        try:
            # ── 检测新食材入锅（RGB+IR联合检测）──────────────────────────────
            if AUTO_DETECT_ENABLED and chunk_i > 0:  # 跳过第一批（已有初始标注）
                # 读取本批第一帧RGB
                cap_detect = cv2.VideoCapture(video_path)
                cap_detect.set(cv2.CAP_PROP_POS_FRAMES, chunk_start_abs)
                ret, rgb_curr = cap_detect.read()
                cap_detect.release()
                
                if ret and tracking_state.rgb_prev_frame is not None:
                    # 获取对应的IR帧
                    ir_curr = temp_data[global_local] if temp_data is not None and global_local < temp_data.shape[0] else None
                    
                    if ir_curr is not None and tracking_state.ir_prev_frame is not None and homography is not None:
                        # 联合检测
                        detected, new_fg, label = detect_new_food_combined(
                            rgb_curr, tracking_state.rgb_prev_frame,
                            ir_curr, tracking_state.ir_prev_frame,
                            homography,
                            rgb_diff_threshold=RGB_DIFF_THRESHOLD,
                            ir_temp_diff_min=IR_TEMP_DIFF_MIN,
                            min_area=RGB_MIN_AREA,
                            aspect_ratio_max=RGB_ASPECT_RATIO_MAX
                        )
                        
                        if detected:
                            print(f"[新食材检测] 批次{chunk_i+1} 检测到{label}，追加{len(new_fg)}个前景点")
                            tracking_state.new_food_detected_count += 1
                            # 将新食材点追加到本批的注入列表（在第0帧注入）
                ...
                # 更新历史帧
                if ret:
                    tracking_state.rgb_prev_frame = rgb_curr
                if ir_curr is not None:
                    tracking_state.ir_prev_frame = ir_curr
            
            # 初始化历史帧（第一批）
            if chunk_i == 0:
                cap_init = cv2.VideoCapture(video_path)
                cap_init.set(cv2.CAP_PROP_POS_FRAMES, chunk_start_abs)
                ret, rgb_init = cap_init.read()
                cap_init.release()
                if ret:
                    tracking_state.rgb_prev_frame = rgb_init
                if temp_data is not None and global_local < temp_data.shape[0]:
                    tracking_state.ir_prev_frame = temp_data[global_local]
            
            # ── 计算本批内哪些帧需要注入额外关键帧 ──────────────────────────
            inject_this_chunk = inject_this_chunk if 'inject_this_chunk' in locals() else []
            ...
            # ── 检查mask质量并自动恢复 ────────────────────────────────────
            if AUTO_RECOVER_ENABLED and carry_mask is not None:
                last_mask = carry_mask
                status = check_mask_quality(tracking_state, last_mask, chunk_i)
                
                if status == "LOST":
                    print(f"[追踪丢失] 批次{chunk_i+1} mask连续低于阈值，启动自动恢复")
                    
                    # 自动生成前景点
                    recover_fg, recover_bg, success = auto_recover_tracking(tracking_state)
                    
                    if success:
                        # 重跑本批（用恢复点替换原始标注）
                        print(f"[重跑] 批次{chunk_i+1} 使用{len(recover_fg)}个恢复点")
                        chunk_masks, carry_mask = track_chunk(
                            predictor, tmp_dir, frame_names,
                            recover_fg, recover_bg,
                            carry_mask=None,  # 重置，用新点初始化
                            inject_keyframes=[]
                        )
                        
                        # 重新检查
                        last_mask = carry_mask if carry_mask is not None else chunk_masks.get(len(frame_names) - 1)
                        status = check_mask_quality(tracking_state, last_mask, chunk_i)
                        print(f"[恢复结果] {status}")
                    else:
                        print(f"[恢复失败] 无法生成恢复点，继续下一批")
                
                elif status == "WARN":
                    print(f"[警告] 批次{chunk_i+1} mask质量下降，继续监控")
                
                # 输出状态摘要
                print(f"[状态] {tracking_state.get_status_summary()}")
